export class AuthUtils {
  ...
  private static matchRolesWithRequested(
    roles: any,
    roleName: string
  ): boolean {
    if (Array.isArray(roles)) {
      const arr = roles.map((element) => {
        return element.toLowerCase();
      });
      return arr.some((x) => x === roleName.toLowerCase());
    } else {
  ...
  public static checkRole(token: string, roleName: string): boolean {
    if (!token || token === '') {
      return false;
    }
    const decodedToken = this._decodeToken(token);
    ...
    return this.matchRolesWithRequested(
      decodedToken[
        'http://schemas.microsoft.com/ws/2008/06/identity/claims/role'
      ],
      roleName
